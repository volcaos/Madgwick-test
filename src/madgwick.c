...
void read_eulerAngle( float *v )
{
	...
	q[2] = SEq_3;
	q[3] = SEq_4;
	float q0q0 = q[0] * q[0], q1q1q2q2 = q[1] * q[1] - q[2] * q[2], q3q3 = q[3] * q[3];
    v[0] = (atan2(2.0f * (q[0] * q[1] + q[2] * q[3]), q0q0 - q1q1q2q2 + q3q3));
    v[1] = (-asin(2.0f * (q[1] * q[3] - q[0] * q[2])));
    v[2] = (atan2(2.0f * (q[1] * q[2] + q[0] * q[3]), q0q0 + q1q1q2q2 - q3q3));
...
{
	if( isnan(SEq_1) || isnan(SEq_2) || isnan(SEq_3) || isnan(SEq_4) ){
		reset_quaternion();
	}
	if( a_x==0.0f && a_y==0.0f && a_y==0.0f ){
		return;
	}
	if( m_x==0.0f && m_y==0.0f && m_y==0.0f ){
		return;
	}
